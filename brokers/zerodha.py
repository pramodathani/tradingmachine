...
class ZerodhaRestAPI:
    ...
    def __init__(self):
        ...
        self._cache = redis.Redis(host=redis_config['host'], port=redis_config['port'], db=redis_config['db'], decode_responses=True, encoding='utf-8')
...
class ZerodhaWebSocket:
    ...
    def __init__(self, name, instrument_tokens):
        ...
        self._mongo_client = pymongo.MongoClient(mongodb_config['connection_string'])        
        self._mongo_db = self._mongo_client[mongodb_config['db']]
        self._cache = redis.Redis(host=redis_config['host'], port=redis_config['port'], db=redis_config['db'])
    ...
    def on_message(self, ws, message):
        ...
        self._cache.hset('market_data', packets)
...
class ZerodhaWebSocketServer:
    ...
    def __init__(self, numbers_of_securities_per_thread=5000):
        """
        Websocket server.
        """
        self._cache = redis.Redis(host=redis_config['host'], port=redis_config['port'], db=redis_config['db'])
