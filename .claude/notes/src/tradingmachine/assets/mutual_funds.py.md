# src/tradingmachine/assets/mutual_funds.py
...
`mutual_funds` is one of UBI's `CASH_SEGMENTS`, so a scheme is reported in the account's holdings exactly as a share is, and `MutualFund` carries the same six holdings members `tradingmachine.assets.equities.Equity` does, copied into the class rather than shared.
