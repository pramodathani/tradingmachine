# src/tradingmachine/assets/analysis/price_analysis.py

`PriceAnalysis` is the shallow base of the thirteen analysis classes in `src/tradingmachine/assets/analysis/`. It declares `prices`, the one method every analysis method calls, and raises `NotImplementedError` from it, so each analysis module documents what it relies on without importing `tradingmachine.assets.instruments`.
...
PriceAnalysis                    prices() contract
  ...
  └── StrategyBacktests           1  run_backtest
          ▲ all inherited by
      tradingmachine.assets.instruments.Instrument  (supplies the real prices())
```
