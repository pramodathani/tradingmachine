...
## `IncompleteResponseError`, added on 2026-09-26

UBI streams its instrument master and has already sent HTTP 200 by the time a failure part-way through can happen, so a broken answer cannot be recognised by its status code. `IncompleteResponseError` is raised by `InstrumentMasterStream` when the array stops before its closing bracket, is not an array at all, or arrives without its `X-Mapping-Date` header. It has no status code of its own and is not in `EXCEPTION_FOR_STATUS_CODE`. It inherits `UnifiedBrokerInterfaceError`, so a caller that already catches every UBI failure catches this one too.
