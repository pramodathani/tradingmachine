...
This module holds `ExchangeTradedFund` and `InvestmentTrust`. They were ported on 2026-09-20 from `assets/funds.py` in the old tradingmachine project at `/run/media/pramod/6959D90B1DAD7E59/backup_20260910/pramod/Downloads/tradingmachine-master/`. `MutualFund` is the third of the old project's fund classes and lives in `assets/mutual_funds.py`, kept apart for the reason below.
...
Both classes carry the same six holdings members `assets.equities.Equity` does, copied into each rather than shared, which is the standing decision for this family of modules. `exchange_traded_funds` and `investment_trusts` are both in UBI's `CASH_SEGMENTS`, so a holding in either resolves exactly as a share's does, and `cnc` is the right product for both.
...
Everything else about construction, the required identity fields, the re-raised not-found error and the prefixed segment check follows the earlier modules, and the reasoning is in `.claude/notes/assets/equities.py.md`.
