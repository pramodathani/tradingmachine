...
Adding a broker means adding an entry to `BROKER_CODES`. Codes are never reused or renumbered, because old archive files keep the code they were written with. The codes are handed out in the order the streaming subsystem learned to carry each broker: Zerodha is 1, Dhan is 2, Flattrade is 3, Shoonya is 4, Fyers is 5 and Groww is 6.
