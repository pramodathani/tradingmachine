...
The cash files carry seven columns, the equity derivative files ten, the commodity files eleven and the currency file twelve. Stacking them produces the union of thirteen, with nulls where a file did not have that column. Nothing is lost; the exchange column already says which shape each row came from, so no extra provenance column is needed.
