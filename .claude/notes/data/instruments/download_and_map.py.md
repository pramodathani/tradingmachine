...
The token reaches the ingester two ways: from the MongoDB `last_login` collection, where the daily broker login records it, or from `--indmoney-access-token` on the command line, which wins when both are present. `download_one` constructs the IND Money ingester specially, passing the token through, while every other broker is constructed with no arguments. The command line flag survives for the case where the login job has not run or its token has been rejected, so the download can still be driven by hand without putting a second copy of the credential anywhere.
