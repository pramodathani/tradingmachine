# ubi_client/client.py

This module holds `UnifiedBrokerInterface`, a thin wrapper around the REST API of the sibling project `unified_broker_interface` (UBI). It adds no knowledge of UBI's routes: callers pass the path, such as `/api/orders/place`, and get the parsed JSON back. It was adapted from `ubi_client/client.py` in the old tradingmachine project, kept at `/run/media/pramod/6959D90B1DAD7E59/backup_20260910/pramod/Downloads/tradingmachine-master/`, and it adds a `patch` method.
