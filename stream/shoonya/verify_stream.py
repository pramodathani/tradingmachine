"""
Checks that Shoonya's JSON frames are being decoded correctly.
...
Run with: python3 -m stream.shoonya.verify_stream --synthetic

The --against-rest mode is the oracle check: it captures live websocket ticks for a spread of instruments, asks Shoonya's own REST quote endpoint what the same instruments are worth, and compares every value the two share. It also measures the implied price scale exchange by exchange, because the wire's `pp` price precision is read off the messages themselves and this is what settles whether the decoder read it right.

Run with: python3 -m stream.shoonya.verify_stream --against-rest --per-exchange 3 --seconds 12

The --feed argument exists because Shoonya documents no depth feed at all. The capture defaults to depth, which is the superset when it works, and --feed touchline is what tells a socket that will not serve depth apart from a market that is closed.
"""
...
from stream.shoonya import connection, packets
from stream.shoonya.connection import ShoonyaConnection
from stream.shoonya.credentials import websocket_credentials
from stream.shoonya.packets import (
    ASK_ORDER_KEYS,
    ASK_PRICE_KEYS,
    ASK_QUANTITY_KEYS,
    BID_ORDER_KEYS,
...
QUOTE_ENDPOINT = "https://api.shoonya.com/NorenWClientAPI/GetQuotes"
...
def build_depth_ack_message(exchange, token, values, price_precision=None):
    """
    Build a depth acknowledgement frame, the full snapshot one scrip gets on a depth subscribe.

    Args:
        exchange (str): The exchange code, for example "NSE".
        token (str): The scrip token as the wire spells it.
        values (dict): Wire fields keyed by their wire names, covering the snapshot's prices, quantities and depth levels.
        price_precision (int | None): The price precision to put in `pp`, or None to leave it out.
...
def build_authentication_not_ok_message():
    ...
    Returns:
        bytes: The frame.
    """
    return build_message(MESSAGE_TYPE_AUTHENTICATION_ACK, {"uid": "AB1234", "s": "Not_Ok"})


def build_order_update_message():
    """
    Build an order update frame, which a market data connection receives whether it asked or not.

    Shoonya's order update feed needs no subscription: updates arrive on every authenticated socket, so a market data connection has to be able to see one and ignore it.

    Returns:
        bytes: The frame.
    """
    return build_message(MESSAGE_TYPE_ORDER_UPDATE, {"uid": "AB1234", "norenordno": "26090600000001", "status": "COMPLETE"})
...
def check_touchline_ack_fields():
    ...
    The values are taken from the snapshot Shoonya's own documentation prints, with the prices all different on purpose, so reading one wire key as another cannot pass.
    ...
    if tick["exchange_timestamp"] != datetime.fromtimestamp(1670231374):
    ...
    if tick.get("last_traded_quantity") is not None or tick.get("bid_quantities") is not None:
        problems.append("the touchline's level one book was mapped, which would give one scrip two different depth shapes")

    passed = not problems
    return ("touchline snapshot fields", passed, "; ".join(problems) or "every snapshot field landed on its contract key, and pc, poi, toi and the level one book stayed out")
...
def check_partial_update_merges_only_changed_fields():
    ...
    Shoonya documents this as the contract in so many words, which makes getting it wrong a documented failure rather than a subtle one.
    ...
    update_frame = build_touchline_update_message(EXCHANGE, TOKEN, {"lp": "1287.45"})
    ...
    blank_update_frame = build_touchline_update_message(EXCHANGE, TOKEN, {"lp": "1287.45"})
    ...
    if empty_timestamp_merged["exchange_timestamp"] != datetime.fromtimestamp(1670231374):
...
def check_price_precision_scales_the_integer():
    ...
    Shoonya documents pp as 2 for NSE and BSE and 4 for CDS USDINR, so both are exercised here, along with a message carrying no `pp` at all, because the default of 2 is a choice this check pins.

    Returns:
        tuple: A (name, passed, detail) triple.
    """
    equity_frame = build_touchline_ack_message(EXCHANGE, TOKEN, {"lp": "1287.30"}, price_precision=2)
    currency_frame = build_touchline_ack_message("CDS", "13", {"lp": "0.0025"}, price_precision=4)
    missing_precision_frame = build_touchline_ack_message(EXCHANGE, TOKEN, {"lp": "1287.30"})
    ...
    problems = []
    if equity_tick["last_price"] != 128730 or equity_tick["price_divisor"] != 100:
    ...
    if missing_tick["last_price"] != 128730 or missing_tick["price_divisor"] != 100:
...
def check_depth_arrays_are_five():
    ...
    Level five on the ask side is deliberately zero, so trimming empty levels or reading only four would be caught.
    ...
    if tick["bid_prices"] != [128730, 128630, 128530, 128430, 128330]:
    ...
    if tick["ask_prices"] != [128735, 128835, 128935, 129035, 129135]:
...
def check_last_trade_time_reads_both_forms():
    """
    The depth feed's last trade time must decode from a clock string and from epoch seconds alike.

    Shoonya publishes no depth documentation at all, so `ltt`'s format is doubly unknown here: neither its presence nor its shape is stated anywhere. The decoder accepts both forms, and this check pins that it really does, so the first live depth frame settles the question rather than silently producing None.

    Returns:
        tuple: A (name, passed, detail) triple.
    """
    clock_tick = decode_frame(build_depth_ack_message(EXCHANGE, TOKEN, {"ltt": "10:29:58"}, price_precision=2), ARRIVAL_TIME)[0]
    epoch_tick = decode_frame(build_depth_ack_message(EXCHANGE, TOKEN, {"ltt": "1670231374"}, price_precision=2), ARRIVAL_TIME)[0]
    nonsense_tick = decode_frame(build_depth_ack_message(EXCHANGE, TOKEN, {"ltt": "not a time"}, price_precision=2), ARRIVAL_TIME)[0]
    absent_tick = decode_frame(build_depth_ack_message(EXCHANGE, TOKEN, {"ltt": ""}, price_precision=2), ARRIVAL_TIME)[0]

    problems = []
    if clock_tick["last_trade_time"] is None or clock_tick["last_trade_time"].strftime("%H:%M:%S") != "10:29:58":
        problems.append(f"clock string read as {clock_tick['last_trade_time']}")
    if epoch_tick["last_trade_time"] != datetime.fromtimestamp(1670231374):
        problems.append(f"epoch seconds read as {epoch_tick['last_trade_time']}")
    if nonsense_tick["last_trade_time"] is not None:
        problems.append(f"an unparseable last trade time became {nonsense_tick['last_trade_time']} instead of None")
    if absent_tick["last_trade_time"] is not None:
        problems.append(f"an empty last trade time became {absent_tick['last_trade_time']} instead of None")

    passed = not problems
    return ("last trade time reads both wire forms", passed, "; ".join(problems) or "a clock string and epoch seconds both decode, and unparseable or empty values become None")


def check_acks_and_control_messages_decode_to_nothing():
    """
    Acknowledgements, heartbeats and order updates must produce no tick.

    These messages share the socket with market data, and decoding any of them as a tick would put phantom rows in the stream. The order update matters most here, because Shoonya sends those without being asked.

    Returns:
        tuple: A (name, passed, detail) triple.
    """
    frames = [
        build_message(MESSAGE_TYPE_AUTHENTICATION_ACK, {"uid": "AB1234", "s": "Ok"}),
        build_authentication_not_ok_message(),
        build_heartbeat_ack_message(1670231374),
...
def check_wire_message_type_strings():
    ...
    if packets.MESSAGE_TYPE_ORDER_UPDATE != "om":
...
def check_assembler_keys_do_not_cross():
    ...
    """
    assembler = packets.TickAssembler()
    first_frame = build_touchline_ack_message(EXCHANGE, TOKEN, {"lp": "1287.30"}, price_precision=2)
...
def check_non_numeric_token_survives():
    """
    A token that is not a number must survive decoding and keying as it was sent.

    Shoonya's own subscribe example includes `NSE|NIFTY`, an index key with a name where every other scrip has digits. Coercing tokens to integers would fail outright on that key, and this check is what stops the coercion being reintroduced as a tidy-up.

    Returns:
        tuple: A (name, passed, detail) triple.
    """
    assembler = packets.TickAssembler()
    frame = build_touchline_ack_message(EXCHANGE, "NIFTY", {"lp": "24570.10"}, price_precision=2)
    tick = assembler.merge(decode_frame(frame, ARRIVAL_TIME)[0])

    passed = (
        tick["token"] == "NIFTY"
        and isinstance(tick["token"], str)
        and assembler.known_instruments() == [(EXCHANGE, "NIFTY")]
    )
    return ("a named token survives as a string", passed, f"token came back as {tick['token']!r} and keyed as {assembler.known_instruments()}")
...
def check_connection_message_builders():
    ...
    The connection builds the messages the server reads, and the decoder reads the messages the server sends, so the two halves of the protocol never touch the same code path. This check pins the builders' JSON to their literal field names and values, so a renamed field cannot pass the decoder checks unnoticed. The connect message's task key is `t`, not the `ta` Flattrade uses, which is the one place the two Noren brokers' wires are known to differ.

    Returns:
        tuple: A (name, passed, detail) triple.
    """
    connect_message = orjson.loads(connection.connect_message("AB1234", "token123"))
    ...
    problems = []
    if connect_message != {"t": "a", "uid": "AB1234", "actid": "AB1234", "source": "API", "accesstoken": "token123"}:
    ...
    if connection.WEBSOCKET_ROOT != "wss://api.shoonya.com/NorenWSAPI/":
...
def check_connect_acknowledgement_status():
    """
    The connect acknowledgement must be read as success only when it really reports success.

    Shoonya documents the success value as "Ok" while Flattrade documents the same field as "OK", so the comparison ignores case. What must not happen is a refusal being read as success, so both documented refusal spellings are checked as well as the blank and missing cases.

    Returns:
        tuple: A (name, passed, detail) triple.
    """
    accepted = ["Ok", "OK", "ok"]
    refused = ["Not_Ok", "NOT_OK", "not_ok", "", None]

    problems = []
    for status in accepted:
        if not connection.connect_acknowledgement_is_ok(status):
            problems.append(f"{status!r} was read as a refusal")
    for status in refused:
        if connection.connect_acknowledgement_is_ok(status):
            problems.append(f"{status!r} was read as success")

    passed = not problems
    return ("connect acknowledgement status read correctly", passed, "; ".join(problems) or "every spelling of Ok is accepted and every spelling of Not_Ok, blank and missing is refused")


def check_exchange_code_translation():
    """
    The master segment to Shoonya exchange code translation must cover every tradable segment and skip the rest.

    NCX is the branch worth watching: Shoonya publishes an NCDEX scrip file where Flattrade publishes none, so this is the first broker stream whose translation reaches NCDEX at all.
    ...
    expected = [
        ...
        ("ncdex", "ncdex_commodity_futures", "NCX"),
        ("ncdex", "ncdex_commodity_options", "NCX"),
    ]
    for master_exchange, master_segment, exchange_code in expected:
        actual = shoonya_exchange_code(master_exchange, master_segment)
    ...
    for master_exchange, master_segment in [
        ("nse", "nse_fixed_income"),
        ("nse", "nse_mutual_funds"),
        ("nse", "nse_exchange_traded_funds"),
        ("bse", "bse_investment_trusts"),
        ("bse", "bse_uncategorised"),
        ("ncdex", "ncdex_uncategorised"),
        ("unknown", "uncategorised"),
    ]:
        if shoonya_exchange_code(master_exchange, master_segment) is not None:
            problems.append(f"{master_exchange}/{master_segment} should have translated to None")

    passed = not problems
    return ("exchange code translation pinned", passed, "; ".join(problems) or "all tradable segments map to their seven codes, NCX included, and the untradable ones to None")
...
def check_compare_tick_to_quote():
    ...
    Returns:
        tuple: A (name, passed, detail) triple.
    """
    values = {"lp": "1287.30", "h": "1291.80", "l": "1278.50", "v": "980515", "ltq": "25"}
    for level in range(5):
        values[BID_QUANTITY_KEYS[level]] = str(100 + level)
        values[BID_PRICE_KEYS[level]] = str(1287.30 - level)
    ...
    quote = {"stat": "Ok", "lp": 1287.30, "h": 1291.80, "l": 1278.50, "v": "980515", "ltq": "25"}
    for level in range(5):
        quote[BID_PRICE_KEYS[level]] = str(1287.30 - level)
        quote[BID_QUANTITY_KEYS[level]] = str(100 + level)
        quote[ASK_PRICE_KEYS[level]] = str(1287.35 + level)
...
def check_implied_scale_ratio():
    ...
    Returns:
        tuple: A (name, passed, detail) triple.
    """
    first_tick = TickAssembler().merge(decode_frame(build_touchline_ack_message(EXCHANGE, TOKEN, {"lp": "1287.30"}, price_precision=2), ARRIVAL_TIME)[0])
...
SYNTHETIC_CHECKS = [
    ...
    check_last_trade_time_reads_both_forms,
    ...
    check_non_numeric_token_survives,
    check_connection_message_builders,
    check_connect_acknowledgement_status,
...
def shoonya_exchange_code(master_exchange, master_segment):
    """
    Translate an instruments.master exchange and segment into Shoonya's exchange code.

    Shoonya publishes seven scrip files, NSE, NFO, CDS, MCX, BSE, BFO and NCX, one more than Flattrade because it lists NCDEX as well. Equity derivatives ride NFO for NSE and BFO for BSE, currency derivatives ride CDS, commodities ride MCX or NCX depending on the exchange, and indices subscribe on their cash exchange. The segments Shoonya lists but does not feed as tradable scrips, fixed income, mutual funds, exchange traded funds, investment trusts and the uncategorised remainder, translate to None and the cross-check skips them.
    ...
    Returns:
        str | None: Shoonya's exchange code, or None when the cross-check should not subscribe to this segment.
    """
    ...
    if master_exchange == "mcx" and bare_segment in ("commodity_futures", "commodity_options", "commodity_index_futures", "commodity_index_options"):
        return "MCX"
    if master_exchange == "ncdex" and bare_segment in ("commodity_futures", "commodity_options", "commodity_index_futures", "commodity_index_options"):
        return "NCX"
    return None
...
def select_verification_instruments(engine, per_exchange):
    """
    Choose Shoonya instruments to check against Shoonya's own quote endpoint, spanning as many exchange codes as possible.
    ...
    Args:
        ...
        per_exchange (int): How many instruments to take from each Shoonya exchange code.
    ...
    statement = sql_text(
        ...
        "  WHERE b.broker = 'shoonya'"
        "    AND b.mapping_date = (SELECT max(mapping_date) FROM instruments.broker_mappings WHERE broker = 'shoonya')"
    ...
    for broker_token, master_exchange, master_segment, master_shape in rows:
        exchange_code = shoonya_exchange_code(master_exchange, master_segment)
...
def capture_ticks(instruments, seconds, mode):
    """
    Open one live connection, collect whatever it sends, and merge it into complete ticks.

    The depth feed is the superset when it works: it sends everything the touchline does plus the five levels a side the quote endpoint also reports. Shoonya documents no depth feed at all, though, so the mode is a parameter here rather than a constant, and a depth capture that comes back empty is not conclusive until a touchline capture is tried against the same market.

    Outside market hours either capture can come back empty, and the caller must treat that as an inconclusive run rather than a passed one.
    ...
    Args:
        ...
        mode (str): Which feed to subscribe, "touchline" or "depth".
    ...
    Raises:
        stream.shoonya.credentials.ShoonyaCredentialsError: If there is no usable user identifier or access token.
        stream.shoonya.connection.ShoonyaConnectionError: If the connection could not be established.
    """
    frames = []
    ...
    async def capture():
        ...
        feed_connection = ShoonyaConnection(
            uid=uid,
...
def fetch_quotes(uid, access_token, instruments):
    """
    Ask Shoonya's REST quote endpoint what it thinks these instruments are worth.

    The endpoint prices one instrument per request, with the query inside a jData form field and the session token beside it in jKey. Shoonya's rate limits page allows about one request per second per instrument for market data, so the pause between requests is what keeps the run inside it.

    Args:
        uid (str): The Shoonya user identifier, sent inside jData.
...
def compare_tick_to_quote(tick, quote):
    """
    Compare one decoded tick against Shoonya's own quote for the same instrument.
    ...
    Args:
        tick (dict): A decoded, merged tick from stream.shoonya.packets.
...
def run_against_rest(per_exchange, seconds, mode, manual_instruments=None):
    """
    Check decoded websocket ticks against Shoonya's own REST quotes for the same instruments.
    ...
    print(f"Checking {len(instruments)} instruments across {len({i['exchange'] for i in instruments})} Shoonya exchange codes against Shoonya's quote endpoint, on the {mode} feed.")
    print()

    uid, access_token, ticks_by_key = capture_ticks(instruments, seconds, mode)
    if not ticks_by_key:
        print(f"  NOTE  the websocket delivered no ticks at all on the {mode} feed")
        print("        outside market hours that is the expected answer, and during a session it would mean Shoonya sent no snapshot on subscribe")
        if mode == connection.MODE_DEPTH:
            print("        Shoonya documents no depth feed, so run this again with --feed touchline before concluding anything about the market")
...
def main():
    ...
    Returns:
        None.
    """
    parser = argparse.ArgumentParser(description="Check that Shoonya's websocket frames are decoded correctly.")
    parser.add_argument("--synthetic", action="store_true", help="Run the synthetic decoding checks, which need no network.")
    parser.add_argument("--against-rest", action="store_true", help="Compare decoded websocket ticks against Shoonya's own REST quotes; needs market hours and credentials.")
    parser.add_argument("--per-exchange", type=int, default=3, help="How many instruments to take from each Shoonya exchange code for --against-rest.")
    ...
    parser.add_argument("--feed", choices=[connection.MODE_DEPTH, connection.MODE_TOUCHLINE], default=connection.MODE_DEPTH, help="Which feed --against-rest should subscribe. Depth carries more to compare; touchline is what Shoonya actually documents.")
    ...
    if arguments.against_rest:
        if arguments.synthetic:
            print()
        failures = failures + run_against_rest(arguments.per_exchange, arguments.seconds, arguments.feed, arguments.instrument)
