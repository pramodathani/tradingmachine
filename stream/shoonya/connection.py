"""
One Shoonya websocket connection, from handshake to reconnection.

This owns a single socket and nothing else, exactly as its counterparts for the other brokers do. It connects, authenticates, subscribes in one of the two feed modes, hands every market data frame it reads to a callback, and reconnects when the connection drops. It does not decode frames, write them anywhere, or decide which instruments to carry, all of which belong to the shard that drives it.

Three things here are less obvious than they look. The first is that Shoonya authenticates in-band rather than at the handshake: the socket opens with no credentials at all, the client sends a connect message as its first text frame, and the server answers with an `ak` acknowledgement carrying `s` of either Ok or Not_Ok, so the handshake status codes the binary brokers classify are kept here only for parity. The second is that Shoonya documents no application heartbeat, only that idle connections are dropped; Noren's own client sends the text message `{"t":"h"}` every thirty seconds and Flattrade documents it as required, so this sends it too, and a `hk` acknowledgement coming back is what confirms the guess. The third is that Shoonya states plainly that subscriptions do not survive a dropped socket, so a reconnected connection would sit open and permanently silent unless this class resubscribes on every session, which it does.

Shoonya's rate limits page claims one websocket connection per session. That is a claim under test rather than a rule this module obeys, and the capacity probe is what tests it; nothing here caps the number of connections a caller may open.
"""
...
from stream.shoonya import packets

WEBSOCKET_ROOT = "wss://api.shoonya.com/NorenWSAPI/"
...
class ShoonyaConnectionError(Exception):
    """
    Raised when a websocket connection cannot be established or maintained.
    """


class ShoonyaAuthenticationError(ShoonyaConnectionError):
    """
    Raised when Shoonya rejects the credentials themselves rather than the connection.
...
class ShoonyaConnectionRefusedError(ShoonyaConnectionError):
    """
    Raised when Shoonya accepts the credentials but will not serve this connection.

    Carries the evidence that led to the conclusion, so the supervisor can record why it stopped opening connections rather than only that it did. Shoonya documents a one connection per session limit but no signature for refusing the second one, so the reasons this class knows, a Not_Ok connect acknowledgement on a later session, a handshake status, an early close and an authentication acknowledgement that never arrives, are the complete set of ways it has to say no.
    """
...
def connect_message(uid, access_token):
    ...
    Shoonya takes no credentials in the URL, so this is the first message on the wire. The documentation lists `uid` as the user identifier and `actid` as the account identifier, and for an individual account they are the same client code, which is how Noren's own client sends it. The `source` must match the source used at login, which for this project is always the API.
...
def subscribe_key(instruments):
    ...
    Shoonya identifies an instrument by exchange and token spelled as one string, and joins several with hashes, so the wire form is also this project's identity form and no translation happens here.
...
def connect_acknowledgement_is_ok(acknowledgement_status):
    """
    Say whether a connect acknowledgement's `s` field reports success.

    Shoonya's documentation spells the two values "Ok" and "Not_Ok", while Flattrade spells the same field's success value "OK". The comparison is therefore made without regard to case, because the cost of the two spellings is wildly asymmetric: reading a good acknowledgement as a refusal raises a non-retryable authentication error and kills a working connection, while the refusal values differ from the success value in more than their case and cannot be confused with it either way.

    Args:
        acknowledgement_status (str | None): The `s` field as it arrived on the wire.

    Returns:
        bool: True when the acknowledgement reports success.
    """
    if acknowledgement_status is None:
        return False
    return str(acknowledgement_status).upper() == "OK"
...
class ShoonyaConnection:
    """
    Drives one Shoonya websocket connection and keeps it subscribed.
    ...
    Attributes:
        ...
        text_messages_received (int): Frames that were neither market data nor heartbeat acknowledgements, which are the connect and unsubscribe acknowledgements and the order updates.
    ...
    def _handle_frame(self, frame):
        ...
        Every frame, whatever it carries, is handed to on_frame encoded to UTF-8, because the archive stores what came off the socket and the manifest counts only the frames whose packet counter reports a packet. Frames that are not market data also go to on_text, so order updates reach whoever asked for them without disturbing the market data path.
    ...
    def _raise_for_connect_acknowledgement(self, message):
        ...
        Raises:
            ShoonyaAuthenticationError: On the first session ever.
            ShoonyaConnectionRefusedError: On any later session.
        """
        if not self.ever_connected:
            raise ShoonyaAuthenticationError(
                f"Shoonya refused the connect message with {message.get('s')}: the uid or access token is wrong."
            )
        raise ShoonyaConnectionRefusedError(
            "connect_not_ok",
            f"Shoonya answered the connect message with {message.get('s')} on a later session, which is how it refuses a connection.",
        )
    ...
    async def _authenticate(self, websocket):
        ...
        Raises:
            ShoonyaAuthenticationError: If Shoonya refused the credentials on the first session ever.
            ShoonyaConnectionRefusedError: If the acknowledgement said Not_Ok on a later session, or never arrived within the timeout.
        ...
        while True:
            ...
            if remaining <= 0:
                raise ShoonyaConnectionRefusedError(
                    "connect_ack_timeout",
                    f"Shoonya sent no connect acknowledgement within {AUTHENTICATION_ACK_TIMEOUT_SECONDS:.0f} seconds.",
                )
            ...
            if parsed.get(packets.MESSAGE_TYPE_KEY) == packets.MESSAGE_TYPE_AUTHENTICATION_ACK:
                if not connect_acknowledgement_is_ok(parsed.get("s")):
    ...
    async def _send_subscription(self, websocket):
        ...
        Instruments are sent in batches of one hundred, a choice rather than a documented rule, and the first live run checks that one acknowledgement comes back per scrip, which Shoonya documents as the behaviour to expect. Shoonya states that subscriptions do not survive a dropped socket, so this runs on every connection including reconnections.
    ...
    async def _run_one_session(self, stop_event):
        ...
        Raises:
            ShoonyaAuthenticationError: If Shoonya rejected the credentials.
            ShoonyaConnectionRefusedError: If Shoonya refused this connection, whether by connect acknowledgement, by handshake status, by an early close, or by never acknowledging.
        ...
        try:
            ...
            if status_code in AUTHENTICATION_STATUS_CODES and not self.ever_connected:
                raise ShoonyaAuthenticationError(
                    f"Shoonya rejected the credentials with status {status_code}. The access token is most likely expired or was issued for a different account."
                ) from error
            raise ShoonyaConnectionRefusedError(
                f"handshake_status_{status_code}",
                f"Shoonya refused the handshake with status {status_code}.",
            ) from error
        ...
        try:
            ...
            if open_for < EARLY_CLOSE_SECONDS and code in REFUSAL_CLOSE_CODES:
                raise ShoonyaConnectionRefusedError(
                    f"early_close_{code}",
                    f"Shoonya closed the connection with code {code} after {open_for:.1f} seconds, which is how it refuses a connection it has already accepted.",
                ) from error
            raise
        finally:
    ...
    async def run(self, stop_event):
        ...
        Every reconnection authenticates and resubscribes from scratch, because Shoonya documents that nothing persists across a dropped socket and a reconnected connection would otherwise stay open and permanently silent.

        The backoff starts at two seconds and doubles to a minute, which is the exponential backoff Shoonya's own overview asks for. A connection that is refused rather than dropped should stop the supervisor from opening further connections rather than be retried.
        ...
        Raises:
            ShoonyaAuthenticationError: If the credentials were rejected, which no amount of retrying will fix.
            ShoonyaConnectionRefusedError: If Shoonya refused the connection, so the caller can record that its limit has been found.
            ShoonyaConnectionError: If the reconnection attempt limit was reached, or the first session failed in probe mode.
        """
        ...
        while not stop_event.is_set():
            try:
                ...
                consecutive_failures = 0
                delay = INITIAL_RECONNECT_SECONDS
            except (ShoonyaAuthenticationError, ShoonyaConnectionRefusedError):
                raise
