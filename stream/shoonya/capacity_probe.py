"""
Measures how much Shoonya's websocket actually allows, rather than what it documents.

Shoonya's rate limits page states one websocket connection per session, with every symbol multiplexed over it, and says nothing at all about how many instruments that one connection will carry. That is a harder claim than Flattrade makes and a less useful one: the number that actually constrains this project is the instrument count, and it is the number Shoonya does not publish.

So the documented limit is treated as the hypothesis rather than the rule. Three things are measured. How many instruments one touchline connection will genuinely serve in full, how many connections can be open at once with every earlier one held open, and what one depth connection serves while a touchline connection is held open. If the connection probe confirms a hard limit of one, that is a finding worth having on record rather than an assumption worth building on.

The protocol's clearest way of saying no is the connect acknowledgement answering Not_Ok on a session that is not the first, which the connection driver raises as ShoonyaConnectionRefusedError; a probe that sees it counts the earlier survivors as the answer.

The measurement leans on a property Shoonya documents outright: the number of tk acknowledgements equals the number of scrips in the subscription. A subscription that is accepted and then only partly honoured shows up here as a snapshot that never completes.

Run with: python3 -m stream.shoonya.capacity_probe
"""
...
from stream.shoonya.connection import (
    MODE_DEPTH,
    MODE_TOUCHLINE,
    ShoonyaAuthenticationError,
    ShoonyaConnection,
)
from stream.shoonya.credentials import websocket_credentials
from stream.shoonya.verify_stream import shoonya_exchange_code
...
def live_instrument_keys():
    ...
    The filters matter and all change the answer. Expired contracts are excluded because they will never tick and would make the probe look as though capacity had been consumed by instruments that produce nothing. Segments Shoonya does not feed as tradable scrips, the mutual funds, exchange traded funds, investment trusts, fixed income and the uncategorised remainder, are dropped because no subscription can carry them. Keys are de-duplicated on the "EXCHANGE|TOKEN" string itself, because two master instruments can share one wire key and subscribing to the same key twice wastes a slot and makes a complete snapshot look incomplete.
...
async def open_and_collect(uid, access_token, instrument_keys, mode, patience_seconds):
    ...
    Args:
        uid (str): The Shoonya user identifier.
    ...
    Raises:
        ShoonyaAuthenticationError: If Shoonya rejected the credentials outright.
    """
    ...
    while waited < patience_seconds:
        ...
        if task.done():
            ...
            if isinstance(exception, ShoonyaAuthenticationError):
                raise exception
...
async def probe_instruments_per_connection(uid, access_token, keys, logger):
    ...
    This is the axis that actually matters for Shoonya. Its documentation asks for every symbol to be multiplexed over one socket without ever saying how many one socket will take, and the live universe is the largest of the four brokers, so the ladder runs all the way to the whole universe.
...
async def probe_connection_count(uid, access_token, keys, ceiling, logger):
    ...
    This is the pass that tests Shoonya's documented one connection per session. Earlier connections are held rather than closed because a limit on simultaneous connections can only be seen by connections that stay open, and because a refusal could also take an earlier connection down with it. When an earlier connection dies the survivors are the true limit, so the probe counts them rather than treating the death as a failure of the connection that caused it.

    A run that stops at one connection has confirmed the documented limit, which is a perfectly good outcome and the one the documentation predicts.
...
async def probe_depth_feed(uid, access_token, keys, logger):
    ...
    Shoonya documents no depth feed at all, so this pass answers a more basic question than its counterparts for the other brokers do: whether a `d` subscription produces anything. Zero delivered instruments with no refusal is the signature of a depth subscription that was accepted and silently not honoured, and outside market hours it is also what a closed market looks like, so a zero from this pass has to be read against the touchline pass in the same run.

    The touchline connection is held throughout so the answer reflects an account that is already streaming rather than an idle one, which matters more here than elsewhere given the documented single-connection limit.
...
async def run_probe(ceiling, store):
    ...
    Raises:
        ShoonyaAuthenticationError: If Shoonya rejected the credentials.
    """
    logger = configure_logging("stream_shoonya_capacity_probe")
    ...
    print(f"  Shoonya documents                               : 1 connection per session")
    print()
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description="Measure Shoonya's real websocket limits.")
