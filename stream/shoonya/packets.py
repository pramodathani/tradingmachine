"""
Decoding of Shoonya's JSON touchline and depth frames.

Every Shoonya market data message is one JSON object carried in one websocket text frame. A subscription is answered with one acknowledgement per scrip, `tk` on the touchline feed and `dk` on the depth feed, which carries the day's full snapshot, and every later message, `tf` and `df`, carries only the fields that changed. So decoding happens in two pieces: `decode_frame` turns one frame into a partial tick holding just the fields the wire sent, and `TickAssembler` keeps the last seen value of every field per scrip and emits the complete tick.

Prices arrive as decimal strings and the snapshot tells the scrip's own price precision in `pp`, so each price is converted to a wire integer by multiplying by ten to the power of that precision. Shoonya documents `pp` as two for NSE and BSE and four for the CDS currency pairs, which is why the divisor is carried per tick rather than fixed at a hundred: a fixed divisor would misstate every currency scrip. The implied scale cross check against REST is what finally settles the precision segment by segment, the same way it settled Zerodha's, Dhan's and Flattrade's scales.

This module knows nothing about sockets, shards, Redis or the database, exactly like its counterparts for the other brokers. It imports `orjson` and `datetime` and nothing else, so it can be tested completely on bytes alone.
"""
...
def parse_message(frame):
    ...
    Args:
        frame (bytes): One complete websocket frame as received, which on Shoonya is one JSON object.
...
def time_of_day_to_datetime(value):
    ...
    The depth feed's `ltt` is undocumented on Shoonya, whose published pages cover the touchline feed alone. A number is read as seconds since the Unix epoch, and a value containing colons is read as a time of day on the day of arrival, which is the only date it could mean.
...
def decode_touchline_message(message, arrival_time):
    ...
    A `tk` acknowledgement carries the day's full snapshot and a `tf` update carries only the fields that changed, so this function maps whichever fields the message actually carries and leaves the rest to TickAssembler. The wire keys are Noren's own compact names: `lp` for the last price, `v` for volume, `o`, `h`, `l` and `c` for the day's prices, `ap` for the average traded price, `oi` for open interest and `ft` for the feed time. The touchline's top of book fields `bq1`, `bp1`, `sq1` and `sp1` are not mapped, because the depth feed reports the same book with five levels and one scrip must not carry two different shapes.
...
def decode_depth_message(message, arrival_time):
    ...
    The depth acknowledgement carries the day's prices, the totals and five levels a side. Shoonya publishes no depth documentation at all, so every key this reads is the Noren key Flattrade documents, and the first live depth subscription is what confirms them. The wire's `c` holds the previous day's close, the same quantity the other brokers' decoders store as close_price. The last trade time in `ltt` has no documented format, so time_of_day_to_datetime reads it either way.
...
def decode_frame(frame, arrival_time):
    ...
    Dispatch is by the message type in the `t` field. Touchline and depth messages decode to one partial tick each, and every other message type, the acknowledgements, the heartbeat exchange and the order updates, decodes to nothing at all, because none of them is market data. Raising instead of returning an empty list would be worse than useless here: this runs inside the socket read loop, so one malformed frame would take down a connection carrying thousands of instruments.
...
def frame_packet_count(frame):
    ...
    One Shoonya frame carries one JSON message, so the count is one for a touchline or depth message and zero for everything else. Counting acknowledgements or heartbeat acknowledgements as packets would corrupt the manifest's reconciliation, and a frame that is not JSON at all carries nothing countable.
...
class TickAssembler:
    ...
    Shoonya's touchline and depth updates carry only the fields that changed since the last message, which its own documentation states outright, so no single message describes a scrip. The assembler keeps the last seen value of every contract field per (exchange, token), remembers each scrip's price precision from its acknowledgement, and returns the complete tick with None for every field the scrip has not reported yet.
