...
The client exchanges the api key and secret for an access token, sends every request with that token, reconnects once when the token is refused, and raises a class from `ubi_client.exceptions` for every failed response.
