...
Both trade on the `nse` and the `bse` exactly as a share does. They are quoted, they can be ordered with the ordinary market and limit wrappers, and they can be held in the demat account, so both carry the same holdings members `assets.equities.Equity` does. An order's quantity is a plain count of units, as for a share, rather than the whole number of lots a commodity or currency order needs.

Symbols are readable tickers, such as `NIFTYBEES` for a fund and `EMBASSY` for a trust. A mutual fund is a different thing and lives in `assets.mutual_funds`, because it is subscribed to at its net asset value rather than traded.
