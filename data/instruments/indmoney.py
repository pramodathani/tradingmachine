...
class IndMoneyInstruments(BrokerInstruments):
    ...
    def download(self):
        ...
        Raises:
            ValueError: If no access token is configured, since the endpoint rejects an unauthenticated request.
            requests.HTTPError: If any of the three requests fails.
        """
        access_token = indmoney_configuration["access_token"]
        if not access_token:
            raise ValueError(
                "No IND Money access token configured. Set BLACK_BOX_INDMONEY_ACCESS_TOKEN in .env. "
