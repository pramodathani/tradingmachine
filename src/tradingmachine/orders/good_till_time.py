...
Indian exchanges offer only `day` and `ioc` validity, with nothing in between, so this fills that gap: whatever has filled by `until_time` is kept, and the rest is cancelled.
...
class GoodTillTimeOrder(synthetic_order.SyntheticOrder):
    ...
    Attributes:
        until_time: The str time of day to cancel what has not filled, as `HH:MM` or `HH:MM:SS` India time, later today.
    """
