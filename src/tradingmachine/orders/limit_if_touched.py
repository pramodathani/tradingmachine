...
`trigger_price` here is the level that fires the order, not the order's own trigger, and `limit_price` is deliberately not defaulted to it. It answers HTTP 202 with an `outcome` of `armed` or `scheduled` and sends nothing to a broker until it fires, so keep the `parent_id` from the answer.
