...
2. **The shared client**, `UnifiedBrokerInterface` in `tradingmachine.ubi_client.client`, is the only code that speaks HTTP. Every object in a process sends its requests through the same client, because UBI holds a single access token for the whole application and a second client would log the first one out. The client asks a [token source](../python-api/client.md#token-sources) for its access token, which by default connects on first use; it asks again and retries once on HTTP 401, turns each failure status into its own exception class, reuses its connections across threads, and remembers which [placement mode](placement-modes.md) UBI was last seen in.
...
There is one deliberate exception to the table. The `tradingmachine.ubi_stores` package reads UBI's own Redis and MongoDB directly, never writing to them, because UBI has no REST route for its stored access token and none for many quotes at once. Only a program that imports it does this, and [Read-only market data](../python-api/read-only-market-data.md#reading-ubis-stores-directly) explains when to.
